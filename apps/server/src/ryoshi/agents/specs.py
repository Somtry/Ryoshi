"""Generative UI 的输出 spec prompt(图片网格 / 追问问题)。

设计意图:
    对应原项目 lib/render/prompt.ts 的 getImageSpecPrompt /
    getRelatedQuestionsSpecPrompt。前端渲染层(apps/web/lib/render/)
    与原项目逐字节一致,模型只要按 spec 产出 ```spec 围栏块,
    图片网格与追问按钮就会在聊天流里渲染出来。

    spec 块是 JSONL(每行一个 RFC 6902 JSON Patch),前端
    spec-fence-block.tsx 负责求值;持久化走 parts 表的 data-* 通用列。
"""

# 图片内嵌 spec(prompt 段落,与原项目逐条对应)
IMAGE_SPEC_PROMPT = """
...
# 追问问题 spec(prompt 段落,与原项目逐条对应)
RELATED_QUESTIONS_SPEC_PROMPT = """
RELATED QUESTIONS:
