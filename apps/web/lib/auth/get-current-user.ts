...
export async function getCurrentUserId() {
  ...
  if (process.env.ENABLE_AUTH === 'false') {
    // Guard: Prevent disabling auth in Ryoshi Cloud deployments
    if (process.env.RYOSHI_CLOUD_DEPLOYMENT === 'true') {
      throw new Error(
        'ENABLE_AUTH=false is not allowed in RYOSHI_CLOUD_DEPLOYMENT'
      )
    }
