...
export function isCloudDeployment(): boolean {
  return process.env.RYOSHI_CLOUD_DEPLOYMENT === 'true'
}

export async function loadModelsConfig(): Promise<ModelsConfig> {
  if (!isCloudDeployment()) {
    throw new Error(
      'loadModelsConfig is only available when RYOSHI_CLOUD_DEPLOYMENT=true'
    )
  }
...
export function loadModelsConfigSync(): ModelsConfig {
  if (!isCloudDeployment()) {
    throw new Error(
      'loadModelsConfigSync is only available when RYOSHI_CLOUD_DEPLOYMENT=true'
    )
  }
